		...
		if(!this.worldObj.multiplayerWorld) {
			...
			if(var3 <= 0 && var2 < 3.0F || var3 > 0 && var2 < 7.0F) {
				...
				if(this.timeSinceIgnited >= var4) {
					if(this.getPowered()) {
						this.worldObj.createExplosion(this, this.posX, this.posY, this.posZ, RedWaveSystem.isRedCreeper(this) ? 8.5F : 6.0F);
					} else {
						this.worldObj.createExplosion(this, this.posX, this.posY, this.posZ, RedWaveSystem.isRedCreeper(this) ? 5.0F : 3.0F);
					}
